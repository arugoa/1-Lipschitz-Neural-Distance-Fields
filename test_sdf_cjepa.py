...
def get_args():
    ...
    parser.add_argument("-o", "--output-name", type=str, default="training_data",
    ...
    parser.add_argument("-model", "--model", type=str,
                        help="path to trained SDF model checkpoint")
...
def imgs_to_tensor(imgs_np, device):
    ...
    returns: (1, T, C, H, W) float32 tensor in [0, 1]
    """
...
if __name__ == "__main__":
    ...
    for i, fp in enumerate(files[:num_train]):
        ...
        enc_np  = encode(savi_model, imgs_np, device)
    ...
    # ── 7. Load SDF model and evaluate ────────────────────────────────────
    print(f"Loading SDF model from {args.model} ...")
    sdf = load_model(args.model, device)
    sdf.eval()

    X_test = torch.from_numpy(np.load(mm_path_test))
    y_test = torch.from_numpy(np.load(mm_path_yt))

    with torch.no_grad():
        preds = sdf(X_test.to(device)).squeeze(-1)
    ...
    accuracy  = (torch.sign(preds) == torch.sign(y_test)).float().mean()
    precision = tp / (tp + fp + 1e-8)
    recall    = tp / (tp + fn + 1e-8)
    f1        = 2 * precision * recall / (precision + recall + 1e-8)

    print(f"\nAccuracy : {accuracy.item() * 100:.4f}%")
